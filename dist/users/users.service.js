...
let UsersService = class UsersService {
    ...
    async create(user) {
        ...
        if (await this.usersRepository.findOneBy({ email: user.email })) {
            throw new common_1.HttpException('A user with that email already exists', common_1.HttpStatus.BAD_REQUEST);
        }
    ...
    async update(id, updateUserDto) {
        ...
        if (anotherUserEmailAlreadyExists) {
            if (anotherUserEmailAlreadyExists.email !== userExists.email)
                throw new common_1.HttpException('Wrong email for current user', common_1.HttpStatus.BAD_REQUEST);
        }
    ...
    async updateToken(id, updateUserDto) {
        const userExists = await this.usersRepository.findOneBy({ id });
        if (!userExists) {
            throw new common_1.HttpException('User not found', common_1.HttpStatus.NOT_FOUND);
        }

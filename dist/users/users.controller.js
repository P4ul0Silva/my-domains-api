...
let UsersController = class UsersController {
    ...
    async findAll() {
        ...
        return (0, class_transformer_1.plainToInstance)(user_entity_1.User, users);
    }
    async findOne(id) {
    ...
    async update(req, updateUserDto) {
        ...
        if (userId.length < 36) {
            throw new common_1.HttpException('Invalid user ID, must be a valid 32 character UUID string', common_1.HttpStatus.NOT_FOUND);
        }
        const updatedUser = await this.usersService.update(userId, { name, email });
        return (0, class_transformer_1.plainToInstance)(user_entity_1.User, updatedUser);
    }
    remove(req) {
        const user = req.user;
